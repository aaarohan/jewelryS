...
const cart = () => {
    ...
    iconCart.addEventListener('click', () => {
        offCanvas.classList.toggle('activeTabCart');
    })
    ...
    const addCartToHTML = () => {
        ...
        if(cart.length > 0){
            cart.forEach(item => {
                ...
                newItem.innerHTML = `
                    <div>
                        <h6 class="my-0 element-title">${info.name}</h6>
                    </div>

                    <div>
                        <small class="text-body-secondary">${info.categories}</small>
                    </div>

                    <span class="text-body-secondary">$${info.price}</span>
